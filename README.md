# Cozmoai TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/cozmoai.svg?label=npm%20(stable)>)](https://npmjs.org/package/cozmoai) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/cozmoai)
...
```sh
npm install git+ssh://git@github.com:CozmoXAI/js-sdk.git
```
...
1. Via the `COZMOAI_LOG` environment variable
...
const client = new Cozmoai({
  logger: logger.child({ name: 'Cozmoai' }),
...
const client = new Cozmoai({ fetch });
```
...
```ts
import Cozmoai from 'npm:cozmoai';
