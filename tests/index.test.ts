...
import { APIPromise } from 'cozmoai/core/api-promise';

import util from 'node:util';
import Cozmoai from 'cozmoai';
import { APIUserAbortError } from 'cozmoai';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['COZMOAI_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Cozmoai) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['COZMOAI_LOG'] = 'debug';
      const client = new Cozmoai({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['COZMOAI_LOG'] = 'not a log level';
      const client = new Cozmoai({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'COZMOAI_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Cozmoai({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['COZMOAI_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Cozmoai({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['COZMOAI_BASE_URL'] = 'https://example.com/from_env';
      const client = new Cozmoai({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Cozmoai({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['COZMOAI_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Cozmoai({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Cozmoai({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Cozmoai({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Cozmoai({ apiKey: 'My API Key', fetch: testFetch });
